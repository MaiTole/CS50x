...
class NoteManager {
    ...
    func saveNote(note: Note) {
        ...
        var statement: OpaquePointer? = nil
        if sqlite3_prepare_v2(
            database, "UPDATE notes SET content = ? WHERE rowid = ?", -1, &statement, nil) ==
            SQLITE_OK {
    ...
    func deleteNote(note: Note) {
        ...
        if sqlite3_prepare_v2(database, "DELETE FROM notes WHERE rowid = ?", -1, &deleteStatement, nil) == SQLITE_OK {
            sqlite3_bind_int(deleteStatement, 1, note.id)
